<div class="container">
    ...
    <form id="editForm" onsubmit="saveUniversity(event, {{ $university->id }})" method="POST"
        ...
        <div class="form-group">
            ...
            <img id="preview" src="{{ asset($university->introductory_photo) }}" alt="Ảnh giới thiệu"
                style="display: {{ $university->introductory_photo ? 'block' : 'none' }}; max-width: 200px;">
        ...
        <div class="form-group">
            ...
            <input type="text" name="introduce" class="form-control" value="{{ $university->introduce }}">
        </div>

        <div class="form-group">
            <label for="time">Thời gian</label>
            <input type="text" name="time" class="form-control" value="{{ $university->time }}">
        </div>

        <div class="form-group">
            <label for="location">Địa điểm</label>
            <input type="text" name="location" class="form-control" value="{{ $university->location }}">
        </div>
        <div id="curriculumContentContainer">
            @foreach ($university->curriculum_content as $index => $content)
            <div class="form-group">
        ...
        <div style="display:flex; gap:10px;">
            ...
            <form>
                ...
                <button type="button" onclick="cancelEdit({{ $university->id }})" class="btn btn-danger delete-btn">Hủy</button>
...
<script>
function saveUniversity(event, universityId) {
    event.preventDefault(); // Ngăn chặn hành vi mặc định của form

    var formData = new FormData(document.getElementById('editForm')); // Tạo FormData từ form

    $.ajax({
        url: "{{ route('university.update', '__ID__') }}".replace('__ID__', universityId),
        type: 'POST',
        data: formData,
        ...
        success: function(response) {
            ...
            $('#row-' + universityId).show(); // Hiển thị lại hàng trong bảng
